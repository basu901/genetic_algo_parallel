...
int main(int argc, char** argv) {
  ...
  if( argc != 3 )
    {
      printf("GA for TSP requires 2 argument: Cities data, output flag, crossover, mutation e.g. ./main cities.txt 0 0.3 0.05 \n");
      exit(-1);
    }
  ...
  */
  ...
  popSize = ((cities*10)/num_ranks)*num_ranks;
  ...
  int r_wheel_length = floor((1-crossoverRatio)*popSize/num_ranks);
  ...
  int filesize = sizeof(unsigned char)*cities*popSize;
  MPI_File fh;
  MPI_Status status;
  int bufsize,nchar;
  // Dynamically allocate buf
  unsigned char *buf=NULL;
  bufsize = filesize/num_ranks;
  nchar = bufsize/sizeof(unsigned char);
  
  // Write out the initial population generated
  MPI_File_open(MPI_COMM_WORLD, "initial_population.txt", 
		MPI_MODE_RDWR|MPI_MODE_CREATE,MPI_INFO_NULL, &fh);
  MPI_File_seek(fh,rank*bufsize,MPI_SEEK_SET);
  MPI_File_write(fh, currentGen , nchar,MPI_UNSIGNED_CHAR, &status);
  // Read in the initial population if needed
  MPI_File_seek(fh,rank*bufsize,MPI_SEEK_SET);
  MPI_File_read(fh,currentGen,nchar,MPI_UNSIGNED_CHAR,&status);
  ...
  */
  ...
  // MPI Parallel File I/O
  // Calculate the filesize
  int filesize = sizeof(unsigned char)*cities*popSize*iteration_count;
  MPI_File fh;
  MPI_Status status;
  int bufsize,nchar;
  // Dynamically allocate buf
  unsigned char *buf=NULL;
  bufsize = filesize/num_ranks;
  nchar = bufsize/sizeof(unsigned char)/iteration_count;
  
  // Write out the initial population generated
  sprintf(filename,"output_of_%d_ranks",num_ranks);
  ...
  if(0==rank)
    {
      ...
      //printf("The total runtime for GA is %4.5f\n", t2-t1);
      totaltime += t2-t1;
      printf("The total I/O time for GA is %4.5f\n", totaltime);
